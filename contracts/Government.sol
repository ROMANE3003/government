...
//contract CitizenERC20 deployed at 0x7C5E55968ADF6F64652e2Ade76f8A63E2F58a026
//contract Government deployed at
...
contract Government {
    ...
    mapping(address => uint256) public dateVote;
    ...
    uint256 private _counterIdProposal;

    //Other variables
    uint256 public retirementAge = 67;
    uint256 public majorityAge = 18;
    uint256 public currentMandateTerm;
    ...
    modifier onlyAdults() {
        require(
            citizens[msg.sender].retirementDate < (block.timestamp + (retirementAge - 18) * 52 weeks),
    ...
    {
        require(
            citizens[_candidateAddress].retirementDate < block.timestamp + (retirementAge - majorityAge) * 52 weeks,
        ...
        require(
            citizens[_candidateAddress].nbOfCurrentAccountTokens >= _stakeAdmin,
        ...
        require(
            dateVote[msg.sender] < currentMandateTerm - _mandateDuration,
    ...
    function updateMandate() public onlySovereign {
        currentMandateTerm = block.timestamp + _mandateDuration;
    }
    ...
    function setAdmin(address _adminAddress) public onlySovereign {
        require(
            citizens[_adminAddress].nbVotes >= _nbMinimumVotesToGetElected,
            "candidate has not received the minimum number of votes"
        );
        require(
            citizens[_adminAddress].nbOfCurrentAccountTokens >= _stakeAdmin,
            "candidate doesn't have enough CTZ to be elected"
        );
    ...
    function proposePolicy(string memory _policy, string memory _description) public onlyAdmin {
        _counterIdProposal++;
        uint256 count = _counterIdProposal;
    ...
    function punish(address _sentenced, Punishment _option) public onlyAdmin {
        ...
        if (_option == Punishment.Small) {
            currentBalance = currentBalance > _smallPunishment ? currentBalance - _smallPunishment : 0;
        } else if (_option == Punishment.Moderate) {
            currentBalance = currentBalance > _moderatePunishment ? currentBalance - _moderatePunishment : 0;
        } else if (_option == Punishment.Serious) {
            currentBalance = currentBalance > _seriousPunishment ? currentBalance - _seriousPunishment : 0;
            ...
            citizens[_sentenced].termBanned = block.timestamp + _banishment;
    ...
    function becomeCitizen(
        ...
        uint256 retirementDate = retirementAge >= _age
            ...
            : block.timestamp;
        token.transferFrom(sovereign, msg.sender, _awardCitizenship);
