...
contract Government {
    ...
    struct Citizen{
        ...
        uint nbVotes;//during elections increased if candidate receives votes. If >= 5 named admin and nbVotes is reset to 0
        uint termAdmin; // till when a citizen is an admin (8 weeks from election)
        uint retirementDate;
        uint termBanned; // till when is the member banned following punishment for treason: block.timestamp + 10 * 52 * 1 weeks
        ...
        uint nbOfUnemploymentTokens;//10% from each salary, acces to it when admins declares him unemployed 
    ...
    mapping (address => uint ) public dateVote; 
    ...
    uint public majorityAge = 18;
    uint public currentMandateTerm;
    uint private mandateDuration = 8 weeks;
    uint private electionsDuration = 1 weeks;
    uint private nbMinimumVotesToGetElected = 5;
    ...
    uint private banishment = 10 * 52 weeks;
    ...
    constructor(address _tokenAddress, uint _price ) public {
        ...
        sovereign = CitizenERC20(_tokenAddress).getOwner();
    ...
    modifier onlySovereign() {
        require(msg.sender == sovereign, 'ERC20: Only sovereign can perform this action');
        _;
    }
    
    /// @dev modifier to check if admin
    modifier onlyAdmin (){
        require (citizens[msg.sender].termAdmin >= block.timestamp, "only admin can perform this action");
        _;
    }
    
    /// @dev modifier to check if citizen has at least 1 unit of CTZ   
    modifier onlySolventCitizens (){
        require (citizens[msg.sender].nbOfCurrentAccountTokens > 0, "only citizens with at least 1 unit of CTZ in current account can perform this action");
        _;
    }

    /// @dev modifier to check if citizen is not banned       
    modifier onlyAllowedCitizens (){
        require (citizens[msg.sender].termBanned < block.timestamp, "only citizens not banned can perform this action");
        _;
    }
    ...
    modifier onlyAdults (){
        require (citizens[msg.sender].retirementDate < (block.timestamp + (retirementAge - 18) * 52 weeks), "only adults can perform this action");
        _;
    }
 
    /// @dev modifier to check if company registered  
    modifier onlyCompanies (){
        require(companies[msg.sender] == true, "Only a company can perform this action");
        _;
    }
    ...
    function elect (address _candidateAddress) public onlyAdults onlyAliveCitizens onlySolventCitizens onlyAllowedCitizens{
        require (citizens[_candidateAddress].retirementDate < block.timestamp + (retirementAge - majorityAge) * 52 weeks, "only adults can be elected");
        require (block.timestamp >= currentMandateTerm - 1 weeks, "too early to elect");
        ...
        require (citizens[_candidateAddress].termBanned < block.timestamp, "candidate is banned");
        require (dateVote[msg.sender] < currentMandateTerm - mandateDuration, "citizen already voted for this election");
    ...
    function setAdmin (address _adminAddress) public onlySovereign {
        require (citizens[_adminAddress].nbVotes >= nbMinimumVotesToGetElected, "candidate has not received the minimum number of votes");
        require (citizens[_adminAddress].nbOfCurrentAccountTokens >= stakeAdmin, "candidate doesn't have enough CTZ to be elected");
        require (citizens[_adminAddress].isAlive == true, "candidate is not a citizen");
        require (citizens[_adminAddress].termBanned < block.timestamp, "candidate is banned");
    ...
    function votePolicy (uint _id, Option _voteOption ) public onlyAdmin {
        require (proposals[_id].didVote[msg.sender] == false, "admin already voted for this proposal");
    ...
    function punish (address _sentenced, Punishment _option ) public onlyAdmin {
        ...
        require (citizens[_sentenced].isAlive == true, "impossible punishment: not an alive citizen");
        ...
        if (_option == Punishment.Small) {
           ...
           currentBalance = currentBalance > seriousPunishment ? currentBalance - seriousPunishment : 0;
            ...
            token.transferFrom(_sentenced, sovereign, token.balanceOf(_sentenced)); 
    ...
    function changeHealthStatus (address _concerned, HealthStatus _option) public onlyAdmin {
        if (_option == HealthStatus.Died){
            ...
            token.transferFrom(_concerned, sovereign, token.balanceOf(_concerned)); 
            ...
            citizens[_concerned].nbOfCurrentAccountTokens += citizens[_concerned].nbOfHealthInsuranceTokens;
            citizens[_concerned].nbOfHealthInsuranceTokens = 0;
    ...
    function changeEmploymentStatus (address _concerned) public onlyAdmin {
        if (citizens[_concerned].isWorking == true){
            ...
            citizens[_concerned].nbOfCurrentAccountTokens += citizens[_concerned].nbOfUnemploymentTokens;
            citizens[_concerned].nbOfUnemploymentTokens = 0;
        } else {
    ...
    function registerCompany(address _companyAddress) onlyAdmin public{
        require (companies[_companyAddress] == false, "company is already registered");
    ...
    function becomeCitizen(uint _age, bool _isWorking, bool _isSick) public{
        require (citizens[msg.sender].retirementDate == 0, "citizens can not ask again for citizenship");
        uint retirementDate = retirementAge >= _age ? block.timestamp + (retirementAge - _age) * 52 weeks : block.timestamp;
        token.transferFrom(sovereign, msg.sender, awardCitizenship);
        citizens[msg.sender] = Citizen(true, address(0), _isWorking, _isSick, 0, 0, retirementDate, 0, awardCitizenship, 0, 0, 0);
    }
    
    /// @dev function to ask for retirement     
    function getRetired() public onlyAliveCitizens onlyAllowedCitizens{
        require (citizens[msg.sender].retirementDate <= block.timestamp, "retirement possible only at 67");
        citizens[msg.sender].isWorking = false;
        citizens[msg.sender].nbOfCurrentAccountTokens += citizens[msg.sender].nbOfRetirementTokens;
    ...
    function buyTokens(uint256 nbTokens) public payable onlyCompanies returns(bool){
        ...
        require(nbTokens >= (10 ** uint256(token.decimals()) / price), "minimum 100 tokens");
        //check if enough ether for nbTokens
        require(nbTokens * price / 10 ** uint256(token.decimals()) <= msg.value, "not enough Ether to purchase this number of tokens");
    ...
    function recruit(address _employee) onlyCompanies public{
        require (citizens[_employee].employer != msg.sender, "employee already working for this company");
        citizens[_employee].employer = msg.sender;
    }

    /// @dev function for a company to pay salaries    
    function paySalary(address payable _employee, uint _amount) onlyCompanies public{
        require (citizens[_employee].employer == msg.sender, "not an employee of this company");
        require (token.balanceOf(msg.sender) >= _amount, "company balance is less than the amount");
        ...
        citizens[_employee].nbOfCurrentAccountTokens = token.balanceOf(_employee) 
